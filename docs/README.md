...
- **[`reference/`](reference/)** — lookup material beyond the spec: `spec/RUST-BINDING-TRACEABILITY.md`
  (invariant → ADR + module), `rust-checklist.md` (idiomatic-Rust/CLI gate), `external-facts.md`
  (perishable web facts with revalidation dates), `reconciliation-report.md`, and
  `known-issues/` (`KI-<NNNN>`).
- **[`guides/`](guides/)** — task docs: `contributing-rust.md` (dependency-management rule),
  `adding-a-subcommand.md` (the four-edit walkthrough).
